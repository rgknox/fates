...
df = pd.read_csv('v4_training_p2.csv', names=['junk','par','vcmax','jmax','gs2','Tl','Patm','Pco2','Esat','gb','E','Kco2','Ko2','Pcc','Rl','Anet','gs'])

# Display the first few rows of the DataFrame
print(df.head())

# Perform data analysis operations
# e.g., print descriptive statistics
print(df.describe())

df_in = df[['par','vcmax','jmax','gs2','Tl','Patm','Pco2','Esat','gb','E','Kco2','Ko2','Pcc','Rl']]
df_out = df[['Anet','gs']]
...
#df = df.iloc[:, df.columns != df.columns[16]]
#df_filtered = df.iloc[:, df.columns != df.columns[[0,15,16]]]
#df_filtered = df.drop(columns=0)
...
model_in = df_in.to_numpy()
model_out = df_out.to_numpy()


#nn_model = 'c3psn_modelsd_szv2_i13_13-L64-Re-L32-Re-2_c20250825-1950.pt'
...
for param in pft_root.iter('param'):
  ...
  for pft in range(numpft):
    ...
    if(param.attrib['name'].strip() == 'fates_leaf_stomatal_intercept'):
      leaf_stomatal_intercept.append(float(param.text.split(',')[pft]))
    if(param.attrib['name'].strip() == 'fates_leaf_stomatal_slope_medlyn'):
      leaf_stomatal_slope.append(float(param.text.split(',')[pft]))
...
for ip in range(n_data):

  par_abs = model_in[ip,0]
  vcmax = model_in[ip,1]
  jmax  = model_in[ip,2]
  gs2   = model_in[ip,3]
  leaf_tempk = model_in[ip,4]
  can_press = model_in[ip,5]
  co2_ppress = model_in[ip,6]
  esat = model_in[ip,7]
  gb = model_in[ip,8]
  vpress = model_in[ip,9]
  mm_kco2 = model_in[ip,10]
  mm_ko2 = model_in[ip,11]
  co2_cpoint = model_in[ip,12]
  lmr = model_in[ip,13]

  o2_ppress = 0.2095*can_press

  gs0 = leaf_stomatal_intercept[0]
  gs1 = leaf_stomatal_slope[0]
  ...
  #print(model_out[ip,0],agross_f.value,(model_out[ip,0]-agross_f.value))
if(False):
...
script_module.save("./c3psn_modelsd_mv4_i14_{}_c{}.pt".format(mod_pattern,datestr))
