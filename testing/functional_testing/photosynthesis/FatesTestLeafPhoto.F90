program FatesTestLeafPhoto
  ...
  real(r8)                           :: canopy_air_press     ! canopy air pressure [Pa]
  real(r8)                           :: can_o2_partial_press ! partial pressure of O2 in the canopy [Pa]
  real(r8)                           :: veg_tempk            ! vegetation temperature [K]
  real(r8)                           :: canopy_air_tempk     ! canopy air temperature [K]
  real(r8)                           :: canopy_air_vpress    ! canopy air vapor pressure [Pa]
  real(r8)                           :: veg_esat             ! saturation vapor pressure at vegetation surface [Pa]
  real(r8)                           :: leaf_bl_resistance   ! leaf boundary layer resistance [s/m]
  ...
  call GetCanopyGasParameters(canopy_air_pressure, can_o2_partial_pressure, veg_tempk,   &
    canopy_air_tempk, canopy_air_vpress, veg_esat, leaf_bl_resistance, mm_kco2, mm_ko2,  & 
    co2_compensation_point, cf, leaf_bl_conductance, constrained_air_vpress)                      
  
  
end program FatesTestLeafPhoto

! subroutine LeafLayerTest
