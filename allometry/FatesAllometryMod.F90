...
module FatesAllometryMod
  ...
  ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! CHANGE SO THAT THIS MODULE ACCEPTS THE RESULT OF GETCROWNREDUCTION
  ! 
  ...
  type :: allom_param_type
     ...
     real(r8), allocatable :: dbh_maxheight(:)        ! dbh at which height growth ceases
     real(r8), allocatable :: hmode(:)                ! height allometry function type
     real(r8), allocatable :: lmode(:)                ! maximum leaf allometry function type
     real(r8), allocatable :: fmode(:)                ! maximum root allometry function type
     real(r8), allocatable :: amode(:)                ! AGB allometry function type
     real(r8), allocatable :: cmode(:)                ! Coarse root allometry function type
     real(r8), allocatable :: smode(:)                ! sapwood allometry function type
     real(r8), allocatable :: stmode(:)               ! storage allometry functional type
     ...
     real(r8), allocatable :: la_per_sa_int(:)        ! Leaf area to sap area conversion, intercept 
     ! (sapwood area / leaf area) [cm2/m2]
     real(r8), allocatable :: la_per_sa_slp(:)        ! Leaf area to sap area conversion, slope 
     ! (sapwood area / leaf area / diameter) [cm2/m2/cm]
     real(r8), allocatable :: l2fr(:)                 ! Fine root biomass per leaf biomass ratio [kgC/kgC]
     ! FOR C-ONLY: this is the static, unchanging ratio
     ! FOR CNP: this is the initial value a cohort starts with
     real(r8), allocatable :: agb_frac(:)             ! Fraction of stem above ground [-]
     real(r8), allocatable :: d2h1(:)                 ! Parameter 1 for d2h allometry (intercept, or "c")
     real(r8), allocatable :: d2h2(:)                 ! Parameter 2 for d2h allometry (slope, or "m")
     real(r8), allocatable :: d2h3(:)                 ! Parameter 3 for d2h allometry (optional)
     real(r8), allocatable :: d2bl1(:)                ! Parameter 1 for d2bl allometry (intercept)
     real(r8), allocatable :: d2bl2(:)                ! Parameter 2 for d2bl allometry (slope)
     real(r8), allocatable :: d2bl3(:)                ! Parameter 3 for d2bl allometry (optional)
     real(r8), allocatable :: blca_expnt_diff(:)      ! Any difference in the exponent between the leaf
     ! biomass and crown area scaling
     real(r8), allocatable :: d2ca_coefficient_max(:) ! upper (savanna) value for crown 
     ! area to dbh coefficient
     real(r8), allocatable :: d2ca_coefficient_min(:) ! lower (closed-canopy forest) value for crown 
     ! area to dbh coefficient
     real(r8), allocatable :: agb1(:)                 ! Parameter 1 for agb allometry
     real(r8), allocatable :: agb2(:)                 ! Parameter 2 for agb allometry
     real(r8), allocatable :: agb3(:)                 ! Parameter 3 for agb allometry
     real(r8), allocatable :: agb4(:)                 ! Parameter 3 for agb allometry
     ...
     real(r8), allocatable :: zroot_max_dbh(:)        ! dbh at which maximum rooting
     ! depth saturates (largest possible) [cm]
     real(r8), allocatable :: zroot_max_z(:)          ! the maximum rooting depth defined
     ! at dbh = fates_zroot_max_dbh [m]
     real(r8), allocatable :: zroot_min_dbh(:)        ! dbh at which the maximum rooting
     ! depth for a recruit is defined [cm]
     real(r8), allocatable :: zroot_min_z(:)          ! the maximum rooting depth defined
     ! at dbh = fates_zroot_min_dbh [m]
     real(r8), allocatable :: zroot_k(:)              ! scale coefficient of logistic
  end type allom_param_type


  ! The core type of this module that
  ! holds the procedures and the parameters

  type, public :: allom_type

     type(allom_param_type) :: params
   ...
   contains
     ...
     procedure :: h2d => h2d_base     ! Generic height to diameter wrapper
     procedure :: d2h => d2h_base     ! Generic diameter to height wrapper
     procedure :: bagw => bagw_base     ! Generic AGWB (above grnd. woody bio) wrapper
     procedure :: bleafmax => bleafmax_base  ! Generic maximum leaf biomass wrapper
     procedure :: bleaf => bleaf_base        ! Generic actual leaf biomass wrapper
     procedure :: bsap  => bsap_base      ! Generic sapwood wrapper
     procedure :: bbgw  => bbgw_base      ! Generic coarse root wrapper
     procedure :: bfnrt => bfnrt_base     ! Generic actual fine root biomass wrapper
     procedure :: bdead => bdead_base     ! Generic bdead wrapper
     procedure :: bstore => bstore_base   ! Generic maximum storage carbon wrapper
     
     procedure :: carea   ! Generic crown area wrapper
     procedure :: storage_fraction_of_target ! storage as fraction of leaf biomass
     procedure :: tree_lai => _base     ! Calculate tree-level LAI from actual leaf biomass
     procedure :: tree_sai => _base     ! Calculate tree-level SAI from tree-level LAI
     
     procedure :: decay_coeff_kn
     procedure :: ForceDBH      ! Method to set DBH to sync with structure
     
   end type allom_type

   type, public, extends(h2d_type) ::h2d_1
   end type h2d_1


   type(allom_type) :: allom
...
contains
  ...
  subroutine h2d_allom(this,h,ipft,d,dddh)

    class(allom_type) :: this
    ...
    associate(  p1   => this%param%d2h1(ipft), &
         p2          => prt_params%d2h2(ipft), &
         p3          => prt_params%d2h3(ipft), &
         allom_hmode => this%params%hmode(ipft))
  ...
  subroutine h_allom(d,ipft,h,dhdd)
    ...
    associate( dbh_maxh => this%params%dbh_maxheight(ipft), &
         p1       => this%params%d2h1(ipft), &
         p2       => this%params%d2h2(ipft), &
         p3       => this%params%d2h3(ipft), &
         allom_hmode => this%params%hmode(ipft))
  ...
  subroutine bagw_allom(d,ipft,crowndamage, elongf_stem, bagw,dbagwdd)
    ...
    associate( p1     => this%params%agb1(ipft), &
         p2           => this%params%agb2(ipft), &
         p3           => this%params%agb3(ipft), &
         p4           => this%params%agb4(ipft), &
         wood_density => this%params%wood_density(ipft), &
         c2b          => this%params%c2b(ipft), &
         agb_frac     => this%params%agb_frac(ipft), &
         allom_amode  => this%params%amode(ipft))
  ...
  subroutine blmax_allom(d,ipft,blmax,dblmaxdd)
    ...
    associate( dbh_maxh    => this%params%dbh_maxheight(ipft), &
         rho         => this%params%wood_density(ipft), &
         c2b         => this%params%c2b(ipft),          &
         allom_lmode => this%params%lmode(ipft),  &
         p1          => this%params%d2bl1(ipft),  &
         p2          => this%params%d2bl2(ipft),  &
         p3          => this%params%d2bl3(ipft))
  ...
  subroutine carea_allom(dbh,nplant,site_spread,ipft,crowndamage,c_area,inverse)
    ...
    associate( dbh_maxh    => this%params%dbh_maxheight(ipft), &
         allom_lmode => this%params%lmode(ipft),  &
         d2bl_p2     => this%params%d2bl2(ipft),  &
         d2bl_ediff  => this%params%blca_expnt_diff(ipft), &
         d2ca_min    => this%params%d2ca_coefficient_min(ipft), &
         d2ca_max    => this%params%d2ca_coefficient_max(ipft))
  ...
  real(r8) function tree_lai( leaf_c, pft, c_area, nplant, cl, canopy_lai, vcmax25top)
    ...
    slat = g_per_kg * this%params%slatop(pft) ! m2/g to m2/kg
    ...
    if(leafc_per_unitarea > 0.0_r8)then
       ...
       sla_max = g_per_kg*this%params%slamax(pft)
  ...
  real(r8) function tree_sai(pft, dbh, crowndamage, canopy_trim, elongf_stem, c_area, nplant, &
    ...
    tree_sai   =  elongf_stem * this%params%sai_scaler(pft) * target_lai
    ...
    if( (treelai + tree_sai) > (sum(dinc_vai)) )then
       ...
       write(fates_log(),*) 'dbh: ',dbh,' dbh_max: ',this%params%dbh_maxheight(pft)
       write(fates_log(),*) 'h: ',h
  ...
  real(r8) function leafc_from_treelai( treelai, pft, c_area, nplant, cl, vcmax25top)
    ...
    slat = g_per_kg * this%params%slatop(pft)
    sla_max = g_per_kg * this%params%slamax(pft)
    ! Coefficient for exponential decay of 1/sla with canopy depth:
    kn = decay_coeff_kn(pft,vcmax25top)
  ...
  subroutine bsap_allom(d,ipft,crowndamage,canopy_trim,elongf_stem, sapw_area,bsap,dbsapdd)
    ...
    select case(int(this%params%smode(ipft)))
       ! ---------------------------------------------------------------------
  ...
  subroutine bbgw_allom(d,ipft,elongf_stem,bbgw,dbbgwdd)
    ...
    select case(int(this%params%cmode(ipft)))
    case(1) !"constant")
  ...
  subroutine bfineroot(d,ipft,canopy_trim,l2fr,elongf_fnrt,bfr,dbfrdd)
    ...
    select case(int(this%params%fmode(ipft)))
  ...
  subroutine bstore_allom(d,ipft,crowndamage, canopy_trim,bstore,dbstoredd)
    ...
    associate( allom_stmode => this%params%stmode(ipft), &
         cushion      => this%params%cushion(ipft) )
  ...
  subroutine bdead_allom(bagw,bbgw,bsap,ipft,bdead,dbagwdd,dbbgwdd,dbsapdd,dbdeaddd)
    ...
    associate( agb_fraction => this%params%agb_frac(ipft))

      select case(int(this%params%amode(ipft)))
  ...
  subroutine bsap_ltarg_slatop(d,h,dhdd,bleaf,dbleafdd,ipft, &
    ...
    associate ( la_per_sa_int => this%params%la_per_sa_int(ipft), &
         la_per_sa_slp => this%params%la_per_sa_slp(ipft), &
         slatop        => this%params%slatop(ipft), &
         wood_density  => this%params%wood_density(ipft), &
         c2b           => this%params%c2b(ipft), & 
         agb_fraction  => this%params%agb_frac(ipft) )
  ...
  subroutine CrownDepth(height,ft,crown_depth)
    ...
    crown_depth = this%params%crown_depth_frac(ft) * height
  ...
  subroutine set_root_fraction(root_fraction, ft, zi, max_nlevroot)
    ...
    select case(nint(this%params%fnrt_prof_mode(ft)))
    case ( exponential_1p_profile_type ) 
       call exponential_1p_root_profile(root_fraction(1:nlevroot), zi(0:nlevroot), this%params%fnrt_prof_a(ft)) 
    case ( jackson_beta_profile_type )
       call jackson_beta_root_profile(root_fraction(1:nlevroot), zi(0:nlevroot), this%params%fnrt_prof_a(ft))
    case ( exponential_2p_profile_type ) 
       call exponential_2p_root_profile(root_fraction(1:nlevroot), zi(0:nlevroot), & 
            this%params%fnrt_prof_a(ft),this%params%fnrt_prof_b(ft))
  ...
  subroutine ForceDBH( ipft, crowndamage, canopy_trim, elongf_leaf, elongf_stem, d, h, bdead, bl )
    ...
    if ( this%params%woody(ipft) == itrue ) then
