...
module FATESPlantRespPhotosynthMod
   ...
   use LeafLevelPhotosynthesisMod, only: LeafLayerPhotosynthesis
   use LeafLevelPhotosynthesisMod, only: rsmax0
...
contains
  ...
  subroutine FatesPlantRespPhotosynthDrive (nsites, sites,bc_in,bc_out,dtime)
    ...
    real(r8) :: lnc                ! Leaf Nitrogen Concentration at the layer of interest [gN/m2]
                     ...
                     if(currentPatch%canopy_mask(cl,ft) == 1)then 
                        ...
                        do iv = 1,currentCohort%nv
                           ...
                           if ( .not.rate_mask_z(iv,ft,cl) .or. &
                              ...
                              ! Use canopy depth scaling to convert top of crown LNC to
                              ! the lnc of the layer
                              lnc = lnc_top * nscaler

                              ! Part VII: Calculate dark respiration (leaf maintenance) for this layer
                              call LeafLayerMaintenanceRespiration( lnc,                      &  ! in
                                                                    ft,                       &  ! in
                                                                    bc_in(s)%t_veg_pa(ifp),   &  ! in
                                                                    lmr_z(iv,ft,cl))             ! out
                              ...
                              if ( (currentPatch%ed_laisun_z(cl,ft,iv) + currentPatch%ed_laisha_z(cl,ft,iv) ) > nearzero ) then


                                 ! Convert the absorbed PAR from Watts/m2 of ground, to W/m2 of leaf
                                 ! 
                                 ! canopy_area_lsl is the fraction that this horizontal leaf layer contributes
                                 ! to the canopy layer. This is because the leaf-layers are calculated in parallel
                                 ! at the PFT level.
                                 ! To get the mean W/m2 of ground absorbed, you would have to add the different
                                 ! crown areas in parallel
                                 ! laisun is the m2 of sunlit leaves per m2 of crown
                                 ! laisha is the m2 of shaded leaves per m2 of crown
                                 ! canopy_area_lsl is the area fraction that this leaf layer occupies
                                 ! compared to all vegetation in the canopy layer
                                 
                                 ! laisun * canopy_area_lsl = [m2 leaf / m2 crown] * [m2 crown / m2 canopy] = [m2 leaf/ m2 canopy]
                                 ! parsun_leaf = [W/m2 canopy] / [m2 leaf / m2 canopy] = [W / m2 leaf]
                                 
                                 ! absorbed_per_sq_m_ground = sum(parsun_lsl(pft))
                                 
                                 parsun_leaf = currentPatch%ed_parsun_z(cl,ft,iv) / &
                                      max(currentPatch%ed_laisun_z(cl,ft,iv) * currentPatch%canopy_area_profile(cl,ft,iv), rsnbl_math_prec)
                                 
                                 parsha_leaf = currentPatch%ed_parsha_z(cl,ft,iv) / &
                                      max(currentPatch%ed_laisha_z(cl,ft,iv) * currentPatch%canopy_area_profile(cl,ft,iv), rsnbl_math_prec)
                              ...
                              rs_z(iv,ft,cl)               = min(rsmax0,cf/(stem_cuticle_loss_frac*stomatal_intercept(ft)))
                              c13disc_z(cl,ft,iv)          = 0.0_r8
