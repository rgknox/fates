...
def CanopyElementPhysics(ican,icol,ico,f90,met,it,xmlroot,pft, \
    ...
    for il in range(elem_diags.n_layer):
        ...
        iret = f90.getabsrad_sub(ci(ican+1),ci(icol+1),ci(visb),c8(avai),c8(avai+davai), \
        ...
        nscaler = np.exp(-kn*(avai+0.5*davai)*elem_diags.total_lai / \
                         (elem_diags.total_lai+elem_diags.total_sai))
